...
export class SchedulingService {
  ...
  generateDailySchedule(date: string): Observable<DailyScheduleResult> {
    return this.http
      .post<DailyScheduleResult>(`${this.schedulingApiUrl}/daily?date=${date}`, null, {
      ...
      .pipe(
        tap((operationPlan) => {
          ...
          console.log(
            '%cOPERATION PLAN - ' + operationPlan.date,
          ...
          console.log(
            '%c✓ Feasible:',
            operationPlan.isFeasible
              ? 'color: green; font-weight: bold'
              : 'color: red; font-weight: bold',
            operationPlan.isFeasible
          );

          const totalVvns = operationPlan.dockSchedules.reduce(
            (sum, dock) => sum + dock.assignments.length,
            0
          );
          console.log(
            '%c🚢 Docks with VVNs:',
            'color: #2196F3; font-weight: bold',
            operationPlan.dockSchedules.length
          );
          console.log('%c📋 Total VVNs:', 'color: #2196F3; font-weight: bold', totalVvns);
          console.log(
            '%c⚠️  Warnings:',
            'color: #FF9800; font-weight: bold',
            operationPlan.warnings.length
          );
          ...
          if (operationPlan.dockSchedules.length > 0) {
            ...
            operationPlan.dockSchedules.forEach((dockSchedule) => {
              console.group(
                `%c🏗️  ${dockSchedule.dockName} (${dockSchedule.assignments.length} VVN(s))`,
  ...
  correctOperationPlan(schedule: DailyScheduleResult): Observable<DailyScheduleResult> {
    return this.http
      .post<DailyScheduleResult>(`${this.schedulingApiUrl}/auto-correct`, schedule, {
        headers: this.getHeaders(),
      })
      .pipe(
        tap((correctedPlan) => {
          console.log(
            '%c🔧 CORRECTED OPERATION PLAN',
            'color: #00BCD4; font-weight: bold; font-size: 16px'
          );
          console.log('Corrected Plan:', correctedPlan);
        })
      );
  }
